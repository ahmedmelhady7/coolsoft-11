...
(function($) {
	...
	$.fn.raty = function(settings) {
		...
		// In functions this not happen because it's become a static value as
		// parameter.
		options = $.extend({}, $.fn.raty.defaults, settings);
		if (settings) {
			ideaUrl = settings['url'] + "&rating=";
		...
		if (options.path
				.substring(options.path.length - 1, options.path.length) != '/') {
			options.path += '/';
		}
		...
		// Local variables to keep the current value and not the last one. Why,
		// Mr. Anderson? Why?
		var id = this.attr('id'), start = 0, starFile = options.starOn, hint = '';
		...
		if (!isNaN(options.start) && options.start > 0) {
			start = (options.start > options.number) ? options.number
					: options.start;
		}
		...
		for ( var i = 1; i <= options.number; i++) {
			...
			hint = (i <= options.hintList.length && options.hintList[i - 1] !== null) ? options.hintList[i - 1]
					: i;

			$global.append(
					'<img id="' + id + '-' + i + '" src="' + options.path
							+ starFile + '" alt="' + i + '" title="' + hint
							+ '" class="' + id + '"/>').append(
					(i < options.number) ? '&nbsp;' : '');
		}

		$(
				'<input id="' + id + '-score" type="hidden" name="'
						+ options.scoreName + '"/>').appendTo($global).val(
				start);
		...
		if (!options.readOnly) {
			if (options.showCancel) {
				var star = $('img.' + id), cancel = '<img src="' + options.path
						+ options.cancelOff + '" alt="x" title="'
						+ options.cancelHint + '" class="button-cancel"/>',

				// Local variables to keep the current value and not the last
				// one in the live functions.
				opt = options, $this = $global;
	...
	function bindAll(context, options) {
		var id = context.attr('id'), score = $('input#' + id + '-score'), qtyStar = $('img.'
				+ id).length;
		...
		$('img.' + id).mouseenter(function() {
			fillStar(id, this.alt, options);
				...
				function() {
					lol = this;
					$.get(ideaUrl + this.alt, function(data) {
						if (data != "failure") {
							score.val(lol.alt);
							$("#idea-rating").html(
									'<font color="#4E4848">Idea rating : '
											+ data + '</font>');
						}
					})
					if (options.onClick) {
	...
	function fixHint(context, score, options) {
		if (score != 0) {
			...
			hint = (score > 0 && options.number <= options.hintList.length && options.hintList[score - 1] !== null) ? options.hintList[score - 1]
					: score;
		} else {
			hint = options.noRatedMsg;
		}

		$('#' + context.attr('id')).attr('title', hint).children('img').attr(
				'title', hint);
	}
	;
	...
	function splitStar(context, score, options) {
		var id = context.attr('id'), rounded = Math.ceil(score), diff = (rounded - score)
				.toFixed(1);

		if (diff >= 0.3 && diff <= 0.7) {
			rounded = rounded - 0.5;
			$('img#' + id + '-' + Math.ceil(rounded)).attr('src',
					options.path + options.starHalf);
		} else if (diff >= 0.8) {
			rounded--;
		} else {
			$('img#' + id + '-' + rounded).attr('src',
					options.path + options.starOn);
		}
	}
	;
