...
(function($) {
	$.fn.el7r_notify = function(options) {
		...
		if(options['place_h']) { var place_h = options['place_h']; } else { var place_h = 'right'; }
		...
		if(options['delay']) { var delay = options['delay']; } else { var delay = '4000'; }
		if(options['effect']) { var effect = options['effect']; } else { var effect = 'fade'; }
		if(options['ex']) { var ex = options['ex']; } else { var ex = true; }
...
/*
option = {
	'text': 'notifyation text',
	'place_v': 'place, top|bottom',
	'place_h': 'place, right|left',
	'icon': 'icon url',
	'skin': 'css class name',
	'delay': 'time to delay'
	'ex': 'x button'
